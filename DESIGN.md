...
## A static UTI is useful beyond preview

Declaring a UTI for an extension has a payoff unrelated to QuickLook: it makes the extension assignable as a default "open with" app. Launch Services' default-handler API (`LSSetDefaultRoleHandlerForContentType`, the call `duti` and similar tools make) refuses two kinds of type with error `-50` – the synthetic `dyn.*` UTI minted for an unrecognized extension, and abstract wildcard UTIs such as `public.data`. So a file whose only type is `dyn.*` (`foo.gemspec`, `bar.ipynb`) or `public.data` (a bare `Dockerfile`, a `.gitignore`) cannot be pointed at an editor by content type at all: there is no concrete, non-wildcard UTI to attach a handler to. Exporting a real `user.*` UTI fixes that – the same declaration that routes the file to a text preview gives it a concrete type, so `duti -s <bundle> user.<ext> all` then succeeds.

Two caveats keep this honest. It unlocks the assignment but does not perform it: an app that already claims the extension at `Owner` rank keeps winning until a default handler is set explicitly. And it does nothing for the `public.data` shapes above (bare names, dot-prefix-only names), which never route through an extension declaration and stay unassignable by type. This is a non-preview motivation for a declaration, but it sits inside the project's remit rather than stretching it: the declaration that makes a text file previewable is the same one that makes it addressable, so an extension added for this reason still earns its place the normal way, with a fixture and a dispatch assertion.
